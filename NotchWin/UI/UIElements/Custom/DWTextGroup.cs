...
{
    internal class NWTextGroup : UIObject
    {
        List<NWText> textPieces = new List<NWText>();

        public float textSize = 24;


        public NWTextGroup(UIObject? parent, string text, Vec2 position, UIAlignment alignment = UIAlignment.TopCenter) : base(parent, position, Vec2.zero, alignment)
        {
            SetText(text);
        }
        ...
        public void SetText(string text)
        {
            ...
            foreach (var c in text)
            {
                ...
                {
                    var textPiece = new NWText(this, c.ToString(), new Vec2(xAdded, 0));
