...
namespace NotchWin.UI.Menu.Menus
{
    internal class VolumeAdjustMenu : BaseMenu
    {
        ...
        private static extern int waveOutGetVolume(IntPtr hwo, out uint NWVolume);

        double GetVolumePercent()
        {
            var volume = (NotchWinMain.defaultDevice == null) ? 1f : NotchWinMain.defaultDevice.AudioEndpointVolume.MasterVolumeLevelScalar;
        ...
        bool IsMuted()
        {
            return (NotchWinMain.defaultDevice == null) ? true : NotchWinMain.defaultDevice.AudioEndpointVolume.Mute;
        }

        DWImage volumeImage;
        UIObject mutedBg;
        NWText muteText;
        NWProgressBar volume;
        ...
        public override List<UIObject> InitializeMenu(IslandObject island)
        {
            ...
            muteText = new NWText(island, "Silent", new Vec2(-15, 0), UIAlignment.MiddleRight);
            muteText.Anchor.X = 1;
            ...
            volume = new NWProgressBar(island, new Vec2(-20, 0), new Vec2(150, 5f), UIAlignment.MiddleRight);
            volume.Anchor.X = 1;
            objects.Add(volume);
